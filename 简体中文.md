...
[标档／MarkDoc（v0.14）](https://GitHub.com/LongTengDao/MarkDoc/)
========================
...
　　在 1.0 版本前，我们临时约定，每次规范内容性的变动（无论是否向下兼容），都会增加次版本号（0.\*）。示例、措辞、排版的调整不一定会改变版本号。  
　　同时，实现的版本也应当以该主次版本号（如 0.N）开头，以表示是对哪一版的实现；实现本身的修改，只能增加末位版本号（0.N.\*）。  
